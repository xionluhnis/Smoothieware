...
void LoadCell::set_gain(char g, bool apply){
    ...
    pin[PIN_CLCK].set(false);
    read();
}

bool LoadCell::is_ready()
{
    return !pin[PIN_DATA].get();
}
...
float LoadCell::read()
{
    ...
    for(char j = 3; j--;) {
        for(char i = 8; i--;){
            pin[PIN_CLCK].set(true);
            // set bit to value
            bool value = pin[PIN_DATA].get();
            char bit = 1 << i;
...
float LoadCell::get_unit(char times)
{
...
void LoadCell::on_get_public_data(void* argument){
    ...
    if(pdr->second_element_is(pressure_checksum)) {
        get_unit(); // read pressure and store it in $last_value
